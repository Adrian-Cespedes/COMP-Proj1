...
void ImpTypeChecker::add_fundec(FunDec* fd) {
    ...
    if (!funtype.set_fun_type(fd->types, fd->rtype)) {
        cout << "Tipo invalido en declaracion de funcion: " << fd->fname
             << endl;
        exit(0);
    }
...
void ImpTypeChecker::visit(FCallStatement* s) {
    ...
    if (funtype.ttype != ImpType::FUN) {
        cout << "No es una funcion" << endl;
        exit(0);
    }

    if (funtype.ttype != ImpType::VOID)
        sp_incr(1);
    ...
    if (num_fun_args != num_fcall_args) {
        cout << "Numero de argumentos no corresponde a declaracion de: "
             << s->fname << endl;
        exit(0);
    }
...
ImpType ImpTypeChecker::visit(FCallExp* e) {
    ...
    if (num_fun_args != num_fcall_args) {
        cout << "(Function call) Numero de argumentos no corresponde a "
                "declaracion de: "
             << e->fname << endl;
        exit(0);
    }
    ...
    for (it = e->args.begin(); it != e->args.end(); ++it) {
        ...
        if (argtype.ttype != funtype.types[i]) {
            cout << "(Function call) Tipo de argumento no corresponde a tipo "
                    "de parametro en fcall de: "
                 << e->fname << endl;
            exit(0);
        }
        i++;
